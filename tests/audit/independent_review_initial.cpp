...
int main(){
 ...
 for(size_t it=0;it<100000;it++){
   ...
   for(char& c:h)if(c>='a'&&c<='f')c=char(c-'a'+'A');auto p2=Sha256Digest::from_hex(h);if(!p2||*p2!=d)fail("uppercase digest parse");
 }
 ...
 // Exercise all documented mesh paths under ASan/UBSan.
 ...
 std::cout << "PASS: 500000 randomized field pairs, 200000 byte encodings, 100000 digest roundtrips, mesh paths; ASan+UBSan clean\n";
}
